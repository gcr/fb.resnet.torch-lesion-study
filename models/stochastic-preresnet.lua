...
local function createModel(opt)
   ...
   -- CIFAR networks
   local function basicblock(n, stride, type, deathRate)
   ...
   local function bottleneck(n, stride, type, deathRate)
      local nInputPlane = iChannels
      iChannels = n * 4
   ...
   local function layer(block, features, count, stride, type)
      local s = nn.Sequential()
      --print(block, features, count, stride, type)
      --print("any of these first?")
      for i=1,count do
         s:add(block(features, i == 1 and stride or 1, type,
                     opt.deathRateScale * (i)/count))
      end
      return s
   end
